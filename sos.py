...
# Razorpay Credentials (Test / Live Keys)
...
def start_handler(message):
    ...
    welcome_text = (
        ...
        "Niche diye gaye buttons se apna platform select karein aur orders lagayein."
    )
...
def process_payment_amount(message):
    try:
        ...
        bot.reply_to(message, f"💳 **Aapka payment link taiyar hai:**\n\n🔗 {short_url}\n\n*Payment poori karne ke baad aapka balance update ho jayega.*", parse_mode="Markdown")
...
def handle_menu_buttons(message):
    ...
    if text == "🛍 Select Platform":
        ...
        fund_text = (
            f"💳 **Add Funds via UPI / Razorpay**\n\n"
            f"1️⃣ QR Code scan karein ya UPI ID par payment karein:\n"
            f"👉 **UPI ID:** `{UPI_ID}`\n\n"
            f"2️⃣ Aap chahe toh `/addfunds` command bhej kar Razorpay link se bhi payment kar sakte hain.\n\n"
        ...
        try:
            ...
            if "order" in res:
                ...
                cursor.execute("INSERT INTO orders VALUES (?, ?, ?, ?, ?, ?, ?)", 
                               (str(res['order']), user_id, selected.get('name'), link, qty, total_cost, now_str))
                conn.commit()
                conn.close()
...
@app.route('/')
def home():
    return "SMM Bot with Razorpay and Webhook is running!"
...
if __name__ == '__main__':
    ...
    # Remove old webhooks to avoid 409 conflict errors
    bot.remove_webhook()
