...
def handle_menu_buttons(message):
    ...
    if text == "🛍 Select Platform":
        ...
        text_msg = f"💳 **Add Funds**\nUPI ID: `{UPI_ID}`\nOr type `/addfunds` for Razorpay link."
        try:
            bot.send_photo(message.chat.id, photo=QR_CODE_URL, caption=text_msg, parse_mode="Markdown")
        except:
...
def platforms_inline_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    # IG Followers Only button added explicitly[span_1](start_span)[span_1](end_span)
    markup.add(types.InlineKeyboardButton("👑 IG Followers Only", callback_data="plat_ig_followers"))
    markup.add(
        types.InlineKeyboardButton("📸 Instagram All", callback_data="plat_instagram"),
        types.InlineKeyboardButton("✈️ Telegram", callback_data="plat_telegram"),
...
def callback_listener(call):
    ...
    if call.data.startswith("plat_"):
        ...
        try:
            ...
            # Specific filter for IG Followers Only button[span_2](start_span)[span_2](end_span)
            if platform_name == "ig_followers":
                for s in services:
                    cat = s.get('category', '').lower()
                    s_name = s.get('name', '').lower()
